...
class AceController extends Controller
{
    ...
    public function index()
    {
        ...
        $leadership  = Leadership::orderby('short','asc')->limit(4)->get();
    ...
    public function loginstaffview(Request $r){
    ...
    public function page($page){
        switch ($page) {
            ...
            case 'investor_relations':
                ...
                $text = $ceo->content;
                $max_length = 1000; // Jumlah maksimum karakter yang diinginkan

                if (mb_strlen($text) > $max_length) {
                    $trimmed_text = mb_substr($text, 0, $max_length) . "...</div>";
                } else {
                    $trimmed_text = $text;
                }
                return view('acewebfront.pages.investor',compact('trimmed_text','ceo','irkey','announcement','download','shareholder','page','announcementnew'));
                break;
            // case 'our_products':
            ...
            case 'newsroom':
                ...
                if($category=='ALL'){
                    $data = Blog::select('blogs.description as description','blogs.date as date','blogs.photos as photos','blogs.id as id','blogs.title as title','blogs.banner as banner','blogs.slug as slug')->join('blog_categories','blog_categories.id','blogs.category_id')->orderby('blogs.date','DESC')->WHERE('category_name','PERSONAL')->paginate(9);
                }else{
                    $data = Blog::select('blogs.description as description','blogs.date as date','blogs.photos as photos','blogs.id as id','blogs.title as title','blogs.title as title','blogs.banner as banner','blogs.slug as slug')->join('blog_categories','blog_categories.id','blogs.category_id')->orderby('blogs.date','DESC')->where('status','1')->where('category_name',$_GET['category'])->paginate(9);
                }
        ...
        switch ($page) {
            ...
            case 'api':
                ...
                if($slug=='getPatternData'){
                    if(isset($_GET['leatherCode']) && isset($_GET['row'])){
                        $data = getPattern($_GET['leatherCode'],$_GET['row']);
                        return $data;
                    }
                }
            ...
            case 'investor_relations':
                if($slug=='message_from_ceo'){
                    ...
                    return view('acewebfront.pages.detail.detailinvestor',compact('data','ceo'));
                }
                break;
            case 'newsroom':
