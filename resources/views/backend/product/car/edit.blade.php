...
<div class="aiz-titlebar text-left mt-2 mb-3">
	<div class="align-items-center">
		<h1 class="h3">{{translate('All Car')}}</h1>
	</div>
</div>
		    ...
		    <div class="card-body">
		        <table class="table aiz-table mb-0">
		            ...
		            <tbody>
                        ...
                        <tr>
                            <td>{{$i+1}}</td>
                            <td><img style="width:200;height:100px" src="{{getimage($v->image)}}" alt="{{$v->name}}"></td>
                            <td>{{$v->name}}</td>
                            <td>{{$v->year}}</td>
                            ...
                            <td>
                                @can('edit_car')
										<a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('cars.edit', ['id'=>$v->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
											<i class="las la-edit"></i>
										</a>
									@endcan
									@can('delete_car')
										<a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('cars.destroy', $v->id)}}" title="{{ translate('Delete') }}">
				...
				<div class="card-body">
					<form action="{{ route('cars.update',$edit->id) }}" method="POST">
						...
						<div class="form-group mb-3">
							...
							<input type="text" placeholder="{{translate('Name')}}" name="name" value="{{$edit->name}}" class="form-control" required>
						</div>
                        <div class="form-group mb-3">
							...
							<select name="year" id="" class="form-control">
                                ...
                                <option value="{{$i}}" @if($edit->year==$i) selected @endif>{{$i}}</option>
                                @endfor
                            </select>
						</div>
                        <div class="form-group mb-3">
							...
							<select name="make" id="" class="form-control">
                                ...
                                <option value="{{$v->id}}" @if($edit->make==$v->id) selected @endif>{{$v->name}}</option>
                                @endforeach
                            </select>
						</div>
                        <div class="form-group mb-3">
							...
							<select name="type" id="" class="form-control">
                                ...
                                @foreach($type as $i =>$v)
                                <option value="{{$v->id}}" @if($edit->type==$v->id) selected @endif>{{$v->type}}</option>
                                @endforeach
                            </select>
						</div>
